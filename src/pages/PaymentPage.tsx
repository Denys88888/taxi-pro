...
import { Check, Shield, Wallet, Loader2, AlertCircle, Lock, Link2, ShieldCheck, RotateCcw, Info } from 'lucide-react';
...
export default function PaymentPage() {
  ...
  const breakdown = useMemo(() => {
    ...
    // Try to restore route info from sessionStorage if context values are 0
    let effectiveDist = routeDistance;
    let effectiveDur = routeDuration;
    if (effectiveDist <= 0) {
      const saved = sessionStorage.getItem('booking_route');
      if (saved) {
        try { const r = JSON.parse(saved); effectiveDist = r.distance || 0; effectiveDur = r.duration || 0; } catch {}
      }
    }
    const distKm = effectiveDist > 0 ? effectiveDist / 1000 : 0;
    const durationMin = effectiveDur > 0 ? effectiveDur / 60 : 0;
    const distanceFee = distKm * 0.10 * tariffMultiplier;
    const timeFee = durationMin * 0.02 * tariffMultiplier;
    const platformFee = price > 0 ? price * 0.02 / 1.02 : 0;
    // Total always matches the price from BookPage context
    const total = price;
  ...
  }, [selectedTariff, routeDistance, routeDuration, price]);
          ...
          {step === 'confirm' && (
            ...
            >
              ...
              >
                ...
                <BreakdownRow
                  label={`${t('distanceFee')} (${breakdown.distKm > 0 ? breakdown.distKm + ' ' + t('km') : '—'})`}
                  value={`${breakdown.distanceFee.toFixed(2)} π`}
                />
                <BreakdownRow
                  label={`${t('timeFee')} (${breakdown.durationMin > 0 ? breakdown.durationMin + ' ' + t('min') : '—'})`}
                  value={`${breakdown.timeFee.toFixed(2)} π`}
                />
                <BreakdownRow label={`${t('platformFee')} (2%)`} value={`${breakdown.platformFee.toFixed(2)} π`} highlight />
                <BreakdownRow label={t('total')} value={`${price.toFixed(2)} π`} isTotal />
              </motion.div>
          ...
          {step === 'escrow' && (
            ...
            >
              ...
              <AnimatePresence>
                {error && (
                  <motion.div
                    className={`mt-4 flex items-center gap-2 text-sm rounded-lg p-3 ${error.includes('Демо') || error.includes('demo') ? 'text-primary bg-primary/10' : 'text-warning bg-warning/10'}`}
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0 }}
                  >
                    {error.includes('Демо') || error.includes('demo') ? <Info size={16} /> : <AlertCircle size={16} />}
                    {error}
                  </motion.div>
                )}
              </AnimatePresence>
