...
export default function PaymentPage() {
  ...
  const steps: { key: PaymentStep; label: string; icon: typeof Check }[] = [
    ...
    { key: 'escrow', label: 'Эскроу', icon: Shield },
  ...
  const handlePayment = useCallback(async () => {
    ...
    try {
      await createPayment(
        price,
        `Taxi Pro: ${pickup.name} → ${destination?.name}`,
        {
        ...
        },
        {
          onReadyForServerApproval: async (paymentId: string) => {
            ...
            try {
              ...
              setError('Ошибка подтверждения платежа. Попробуйте снова.');
            }
          },
          onReadyForServerCompletion: async (paymentId: string, txid: string) => {
            ...
            try {
              ...
              setError('Ошибка завершения платежа. Обратитесь в поддержку.');
            }
          },
          ...
          onError: (paymentId: string, err: Error) => {
            ...
            setError(err.message || 'Ошибка платежа. Попробуйте снова.');
            setStep('confirm');
          },
        }
      );
      ...
      setError('Не удалось создать платеж. Демо-режим...');
      setTimeout(() => {
        setStep('done');
        notifyPaymentRequired(price);
        const driver = findMockDriver();
  ...
  return (
    <div className="absolute inset-0 z-modal-content bg-bg-body flex flex-col">
      ...
      <div className="pt-6 px-4 pb-4">
        <h1 className="text-text-primary text-xl font-bold text-center">Оплата</h1>
        <p className="text-text-tertiary text-sm text-center mt-1">Стоимость поездки в эскроу</p>
      </div>
        ...
        <AnimatePresence mode="wait">
          {step === 'confirm' && (
            ...
            >
              <div className="bg-bg-elevated rounded-piride-xl p-6 text-center border border-white/5 shadow-card">
                <p className="text-text-secondary text-sm mb-1">Общая стоимость</p>
                <motion.p
                ...
                <p className="text-text-tertiary text-xs mt-2">Pi удерживается в эскроу до завершения поездки</p>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 bg-bg-elevated/50 rounded-piride-md p-3 border border-white/5">
                  ...
                  <div>
                    <p className="text-text-primary text-sm font-medium">Оплата Pi</p>
                    <p className="text-text-tertiary text-xs">Оплатить криптовалютой Pi</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 bg-bg-elevated/50 rounded-piride-md p-3 border border-white/5">
                  ...
                  <div>
                    <p className="text-text-primary text-sm font-medium">Защита эскроу</p>
                    <p className="text-text-tertiary text-xs">Средства будут освобождены после завершения поездки</p>
                  </div>
                </div>
              </div>
          ...
          {step === 'escrow' && (
            ...
            >
              ...
              <h3 className="text-text-primary text-lg font-semibold mb-2">Обработка платежа</h3>
              <p className="text-text-secondary text-sm">Подождите, мы защищаем ваш платеж...</p>
          ...
          {step === 'done' && (
            ...
            >
              ...
              <h3 className="text-text-primary text-xl font-bold mb-2">Платеж защищен!</h3>
              <p className="text-text-secondary text-sm mb-1">{price.toFixed(2)} Pi в эскроу</p>
              {txid && <p className="text-text-tertiary text-xs font-mono">TX: {txid.slice(0, 16)}...</p>}
              <p className="text-text-tertiary text-xs mt-4">Ищем водителя...</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
