...
function documentLabel(type: DriverDocument['type']): string {
  switch (type) {
    case 'license': return 'driverLicense';
    case 'insurance': return 'insurance';
    case 'registration': return 'vehicleRegistration';
    case 'background_check': return 'backgroundCheck';
...
function statusLabel(status: DriverDocument['status']): string {
  switch (status) {
  ...
  useEffect(() => {
    ...
    const handleRideAvailable = (data: any) => {
      const newRequest: RideRequest = {
        ...
        passenger: data.passenger || { name: 'Unknown', rating: 5 },
      };
