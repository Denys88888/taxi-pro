...
export default function RidesPage() {
  ...
  const grouped = rideHistory.reduce<Record<string, typeof rideHistory>>((acc, ride) => {
    const date = new Date(ride.createdAt).toLocaleDateString('ru-RU', {
      weekday: 'short',
      month: 'short',
      day: 'numeric',
    });
          ...
          <div className="space-y-6">
            <div>
              <h3 className="text-text-tertiary text-xs font-semibold uppercase tracking-wider mb-3">
                {new Date(Date.now() - 86400000).toLocaleDateString('ru-RU', { weekday: 'short', month: 'short', day: 'numeric' })}
              </h3>
              <div className="space-y-2">
          ...
          >
            <div className="px-4 pb-4 space-y-2 border-t border-white/5 pt-3">
              <div className="flex justify-between text-xs">
                <span className="text-text-tertiary">{t('tripId')}</span>
                <span className="text-text-secondary font-mono">{ride.id.replace('mock_', 'П')}</span>
              </div>
              <div className="flex justify-between text-xs">
