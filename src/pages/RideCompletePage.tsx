  ...
  return (
    <div className="absolute inset-0 z-modal-content bg-bg-body flex flex-col">
      ...
      <div className="flex flex-col items-center pt-12 pb-6 px-6">
        ...
        >
          Thank you for riding with Taxi Pro
        </motion.p>
      </div>
      ...
      <motion.div
        ...
        transition={{ delay: 0.3 }}
      >
        <h3 className="text-text-secondary text-xs font-semibold uppercase tracking-wider mb-3">Trip Summary</h3>
        <div className="space-y-2">
          ...
          <div className="border-t border-white/5 pt-3 mt-3 flex justify-between items-center">
            <span className="text-text-secondary text-sm">Total Paid</span>
      ...
      >
        <h3 className="text-text-secondary text-xs font-semibold uppercase tracking-wider mb-3 text-center">Rate Your Driver</h3>
        <StarRating rating={rating} onRate={setRating} />
        <p className="text-text-tertiary text-xs text-center mt-2">
          {rating === 0 ? 'Tap to rate' : rating >= 4 ? 'Great ride!' : rating >= 3 ? 'Good ride' : 'We\'re sorry'}
        </p>
      </motion.div>
      ...
      >
        <h3 className="text-text-secondary text-xs font-semibold uppercase tracking-wider mb-3 text-center">Add a Tip</h3>
        <div className="flex gap-2 justify-center">
          {TIP_AMOUNTS.map((amount) => (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
            >
              {amount === 0 ? 'None' : `+${amount.toFixed(1)}`}
            </motion.button>
          ))}
        </div>
      </motion.div>
      ...
      <div className="px-4 pb-8 pt-4">
        {submitted ? (
          ...
          >
            ...
            <p className="text-primary text-sm font-medium">Thanks for your feedback!</p>
          </motion.div>
        ) : (
